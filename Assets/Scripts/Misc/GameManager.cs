...
public class GameManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (_input.SwitchCharacterWasPressedThisFrame)
        {
            ...
            _cameraController.cinemachine3rdPerson.GetComponent<Unity.Cinemachine.CinemachineCamera>().Follow = 
                (_redController.enabled ? _redController.CameraFollow : _blueController.CameraFollow);
            _cameraController.cinemachine3rdPerson.GetComponent<Unity.Cinemachine.CinemachineCamera>().LookAt = 
                (_redController.enabled ? _redController.CameraFollow : _blueController.CameraFollow);

            _cameraController.cinemachine1stPerson.GetComponent<Unity.Cinemachine.CinemachineCamera>().Follow = 
                (_redController.enabled ? _redController.CameraFollow : _blueController.CameraFollow);

            _cameraController.cinemachineOrbit.GetComponent<Unity.Cinemachine.CinemachineCamera>().Follow = 
                (_redController.enabled ? _redController.CameraFollow : _blueController.CameraFollow);
            _cameraController.cinemachineOrbit.GetComponent<Unity.Cinemachine.CinemachineCamera>().LookAt = 
                (_redController.enabled ? _redController.CameraFollow : _blueController.CameraFollow);
